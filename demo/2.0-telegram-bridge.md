...
**Scenario**: The agent sends a periodic health check to your Telegram chat,
confirming the sandbox, inference, and network policy are all operational.

Use `openclaw heartbeat` to start a recurring probe that runs inside the
sandbox and reports status via the Telegram bridge.
...
- Periodic messages from the bot confirming the agent is alive
- Inference status (model name, provider)
- Any policy violations or connectivity issues

**What to show**:

- The bot posting regular health updates without manual interaction
- Stopping and restarting the heartbeat to show it recovers
- A failed heartbeat when the sandbox is deliberately stopped
