...
`curl http://172.18.0.1:8000/generate …` every time. The
...
**Fast path — upload from the host** (replace `<sandbox>` with your sandbox name):

```bash
NEMOCLAW_DEMO_ROOT="${NEMOCLAW_DEMO_ROOT:-${HOME}/NemoClaw-Demo}"

openshell sandbox upload "<sandbox>" \
  "${NEMOCLAW_DEMO_ROOT}/demo/skills/aiq-research/SKILL.md" \
...
> If the `172.18.0.1` gateway from Step 3 was different, open the skill file
> before uploading and replace the endpoint IP so it matches your network
> policy.
...
script), see [Demo 2.2 — Install the philosopher-nudge skill](2.2-telegram-hourly-nudge.md#step-2--install-the-philosopher-nudge-skill-in-the-sandbox)
...
- `Do deep research on CUDA unified memory — cite sources.`
...
`http://172.18.0.1:8000/generate`, and reply with an `### Answer` + `### Sources`
...
2. The network policy isn't applied — re-run `nemoclaw nemoclaw policy-add`
   and pick `aiq-local`.
...
1. **User prompt** (e.g. "do deep research on X") matches the **`aiq-research`
   ...
   issues a single `curl` to `http://172.18.0.1:8000/generate`.
2. **OpenShell gateway / proxy** intercepts the request and checks the
   `aiq-local` network policy — `172.18.0.1:8000` is allowed with `access: full`.
...
- [`aiq-research` skill](skills/aiq-research/SKILL.md) · [demo skills README](skills/README.md)
- [Demo 1.0 — Basic sandbox](1.0-basic.md)
- [Demo 2.0 — Telegram Bridge](2.0-telegram-bridge.md)
- [Demo 2.2 — Philosopher nudge skill (install pattern)](2.2-telegram-hourly-nudge.md)
