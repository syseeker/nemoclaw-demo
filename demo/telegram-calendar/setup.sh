...
# Re-deploy Telegram + Calendar integration (restart push daemon, re-upload gog,
# re-apply skill). Use after a reboot or sandbox reset. Skips OAuth and gog
...
echo ""
echo -e "${CYAN}  Telegram + Calendar -- Re-deploy (gog CLI + Push Daemon)${NC}"
echo ""
...
openshell sandbox upload "$SANDBOX" "$SKILL_UPLOAD/calendar" /sandbox/.openclaw/skills/calendar 2>/dev/null || \
  warn "Skill upload warning (non-fatal)"
ok "calendar SKILL.md deployed"
